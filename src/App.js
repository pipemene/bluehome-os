...
const API_URL = process.env.REACT_APP_API_URL || "https://bluehome-os-production.up.railway.app";
...
export default function App() {
  const [route, setRoute] = useState("inquilino");
  return (
    <>
      ...
      <div className="container">
        ...
        <div className="small" style={textAlign:"center",marginTop:16}>API: {API_URL} — Archivos en S3 (si hay credenciales) o base64.</div>
      </div>
    </>
  );
}
...
function AdminBoard(){
  ...
  async function load(){
    ...
    else alert("Inicia sesión para ver órdenes.");
  }
  useEffect(()=>{ load(); },[]);
...
function WorkCard({ order, me, reload }){
  ...
  async function generatePDF(){
    ...
    const margin = 40; let y = margin; const W = doc.internal.pageSize.getWidth();
    const text = (t, inc=14, size=10) => { doc.setFontSize(size); doc.text(t, margin, y); y += inc; };
    doc.setFontSize(16); text("Blue Home Inmobiliaria — Acta de Trabajo", 18, 16);
    text(`Radicado: ${order.radicado}`); text(`Fecha: ${new Date().toLocaleString()}`);
    text(`Técnico: ${me || order.assignedTo || "—"}`); text(`Estado: ${order.status}`);
    text(`Código Inmueble: ${order.tenant?.codigo}`); text(`Inquilino: ${order.tenant?.nombre} — Tel: ${order.tenant?.telefono}`);
    y += 6; doc.setFontSize(12); doc.text("Descripción de la solicitud:", margin, y); y += 12;
    doc.setFontSize(10); const lines = doc.splitTextToSize(order.tenant?.descripcion||"", W - margin*2); doc.text(lines, margin, y); y += 12 + lines.length*10;
    const addGal = (title, arr) => { if(!arr?.length) return; if (y>740){doc.addPage(); y=margin;} doc.setFontSize(12); doc.text(title, margin, y); y+=8; const size=140,gap=10; let x=margin;
      arr.forEach(img=>{ if (y>760){doc.addPage(); y=margin; x=margin;} const src = img.url||img.base64||img.data; doc.addImage(src,"JPEG",x,y,size,size); x+=size+gap; if(x+size>W-margin){x=margin; y+=size+gap;} }); y+=6; };
    addGal("Antes:", before); addGal("Durante:", during); addGal("Después:", after);
    if (y>740) {doc.addPage(); y=margin;} doc.setFontSize(12); doc.text("Materiales utilizados:", margin, y); y+=12;
    doc.setFontSize(10); const m = doc.splitTextToSize(materials||"—", W - margin*2); doc.text(m, margin, y); y+=12+m.length*10;
    if (y>740) {doc.addPage(); y=margin;} doc.setFontSize(12); doc.text("Observaciones del técnico:", margin, y); y+=12;
    ...
    if (email) { try { const r = await authorizedFetch("/api/send-pdf", { method:"POST", body:JSON.stringify({ orderId: order.id, toEmail: email, pdfBase64 }) }); const j = await r.json(); if (j.preview) alert("PDF enviado. Vista previa: "+j.preview); else alert("PDF enviado."); } catch { alert("No se pudo enviar el PDF por email"); } }
    else { alert("PDF generado y orden cerrada (sin email)."); }
  }
